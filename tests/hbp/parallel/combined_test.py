...
def example_layer(cls):
    ...
    set_seeds(0)
    return cls(in_features=in_features, out_features=out_features, bias=True)
...
def test_forward_pass():
    ...
    for cls in test_classes:
        ...
        x = random_input()
        assert torch.allclose(layer(x), parallel(x))
...
def test_input_hessian():
    ...
    for cls in test_classes:
        ...
        assert torch.allclose(in_h, brute_force_input_hessian(cls))
...
def brute_force_parameter_hessian(cls, which):
    ...
    if which == 'weight':
        return [
            exact.exact_hessian(loss, [child.linear.weight])
            for child in layer.parallel_children()
        ]
    elif which == 'bias':
        return [
...
def compare_no_splitting_with_composition(device, cls, num_iters):
    ...
    assert (len(list(layer.parameters())) == len(list(parallel.parameters()))
            == 2)
...
def compare_sgd_optimization_no_split_with_composition(cls, device, num_iters):
    ...
    for i in range(num_iters):
        ...
        assert torch.allclose(layer.linear.weight, parallel.main.linear.weight)
        assert torch.allclose(layer.linear.bias, parallel.main.linear.bias)
...
def test_compare_sgd_optimization_no_split_with_composition_cpu(num_iters=50):
    ...
    device = torch.device('cpu')
    for cls in test_classes:
        compare_sgd_optimization_no_split_with_composition(
            cls, device, num_iters)
...
def compare_newton_optimization_no_split_with_composition(
        cls, device, num_iters):
    """Check if parallel composition layer with splitting of 1 behaves
    ...
    # create optimizers
    opt1 = CGNewton(layer.parameters(), 0.1, 0.05, cg_tol=1E-1, cg_maxiter=50)
    opt2 = CGNewton(
...
def test_compare_newton_optimization_no_split_with_composition_cpu(
        num_iters=50):
    ...
    device = torch.device('cpu')
    for cls in test_classes:
        compare_newton_optimization_no_split_with_composition(
            cls, device, num_iters)


def test_compare_newton_optimization_no_split_with_composition_gpu(
        num_iters=50):
    """Check if parallel composition layer without splitting behaves the same
    as HBPCompositionActivationLinear during optimization with Newton-style
    optimizer on GPU."""
