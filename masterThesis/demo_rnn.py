...
print(
    "All Jacobian J_h_b match?",
    torch.allclose(jac_einsum[2], jac_torch[2])
    and torch.allclose(jac_einsum[2], jac_for[2]),
)
print("Jacobian J_h_ih match?", torch.allclose(jac_torch[0], jac_einsum[0]))
print("Jacobian J_h_hh match?", torch.allclose(jac_torch[1], jac_einsum[1]))
...
print(
    "Are all bias_ih gradients the same?",
    torch.allclose(*igrad_autograd[2:4])
    and torch.allclose(igrad_bias_ih_einsum, igrad_autograd[2])
    and torch.allclose(igrad_bias_ih_einsum, igrad_bias_ih_pytorch)
    and torch.allclose(igrad_bias_ih_einsum, igrad_bias_ih_backpack),
)
...
for n_param in range(4):
    ...
    print(
        f"Is igrad same for param {n_param}?",
