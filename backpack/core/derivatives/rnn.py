...
class RNNDerivatives(BaseParameterDerivatives):
    ...
    * i: Input dimension
    ...
    def _bias_ih_l0_jac_t_mat_prod(self, module, g_inp, g_out, mat, sum_batch=True):
        ...
        T = mat.shape[1]
        H = mat.shape[3]
        output = module.output.cuda()
        jac = zeros(T, N, H, H).cuda()
        for t in range(T):
    ...
    def _weight_ih_l0_jac_t_mat_prod(self, module, g_inp, g_out, mat, sum_batch=True):
        ...
        H = mat.shape[3]
        output = module.output.cuda()
        input0 = module.input0.cuda()
        jac = zeros(T, N, H, H, module.input_size).cuda()
        for t in range(T):
            jac[t, ...] = einsum(
                "nk, kh, nj -> nkhj",
                1 - output[t, ...] ** 2,
