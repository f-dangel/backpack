...
class DiagHLinear(BackpropExtension):
    ...
    def backpropagate_sqrt_hess(self, module, grad_input, grad_output,
                                sqrt_h_outs, sqrt_h_outs_signs):
        for i, sqrt_h in enumerate(sqrt_h_outs):
            sqrt_hess_outs[i] = jac_mat_prod(module, grad_input, grad_output,
                                             sqrt_h)
        CTX._backpropagated_sqrt_h = sqrt_hess_outs
