...
class DiagHSigmoid(BackpropExtension):
    def __init__(self):
        super().__init__(torch.nn.Sigmoid, DIAG_H, req_inputs=[0])
    ...
    def sqrt_h_residuals(self, module, grad_output):
        batch, sigma = grad_output[0].size(0), module.output
        d2_sigma = sigma * (1 - sigma) * (1 - 2 * sigma)
        res = d2_sigma.view(batch, -1) * grad_output[0].view(batch, -1)
