"""Test Hessian backpropagation of reshape layer."""
...
def test_shape():
    ...
    x = randn(10, 20, 3)
    layer = HBPReshape((-1, 5, 2))
    out = layer(x)
    assert tuple(out.size()) == (60, 5, 2)

    # invalid shapes
    x = randn(5, 4)
    layer = HBPReshape((10, 3))
    try:
        out = layer(x)
    except RuntimeError:
        return
...
def example_input(shape, seed=0):
    """Example input for the reshape layer."""
    set_seeds(seed)
    x = randn(*shape)
...
def test_forward_pass():
    ...
    out_1 = x_1.reshape(*target_shape)

    # forward pass in HBP
    x_2 = example_input(shape)
    assert torch_allclose(x_1, x_2)
    layer = HBPReshape(target_shape)
    out_2 = layer(x_2)
